...
class CustomPostType {
	...
	public static function checkEmailTemplateExist( $args = false, $order = null ) {
		...
		if ( false != $args ) {
			...
			if ( null !== $translate_integration ) {
				return $translate_integration::get_template_id( $args, $order );
			}
	...
	public static function templateEnableDisable( $getPostID = true ) {
		...
		$translate_integration = \YayMail\Integrations\Translations\Initialize::get_integration();
		if ( null !== $translate_integration ) {
			$template_export = $translate_integration::get_list_template( $posts, $getPostID );
		} else {
			if ( count( $posts ) > 0 ) {
				$list_use_temp = array();
	...
	public static function insert( $args = false ) {
		if ( false != $args && is_array( $args ) ) {
			...
			if ( 'yaymail_template' == $args['post_type'] ) {
				...
				$translate_integration = \YayMail\Integrations\Translations\Initialize::get_integration();
				if ( null !== $translate_integration ) {
					$translate_integration::update_language_meta( $insert_id );
				}
			}
			return $insert_id;
		}
