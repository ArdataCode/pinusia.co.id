...
} elseif ( isset( $args['custom_order_status'] ) && true == $args['custom_order_status'] ) {
	$template = 'wc-' . $status_to;
} else {
...
if ( is_object( $args['email'] ) && 'WC_GZD_Email_Customer_Cancelled_Order' === get_class( $args['email'] ) && 'customer_failed_order' === $template ) {
...
if ( class_exists( 'WOOCOS_Email_Manager' ) ) {
	if ( isset( $args['order'] ) && null != $args['order']->data['status'] && false !== strpos( $default_path, 'custom-order-statuses-for-woocommerce' ) ) {
...
// support YITH WooCommerce Coupon Email System Premium
...
if ( isset( $args['email'] ) && is_object( $args['email'] ) && 'wc_sc_email_coupon' !== $template && 'wc_sc_combined_email_coupon' !== $template && 'wc_sc_acknowledgement_email' !== $template ) {
...
if ( ( false === $checkIsSumoTemp ) && ( false === $checkIsQWCTemp ) && isset( $args['email'] ) && isset( $args['email']->id ) && ! empty( $args['email']->id ) && isset( $args['order'] ) && $args['order']->get_id() ) {
	...
	if ( isset( $args['email'] ) && isset( $args['email']->id ) && ( 'customer_new_account' === $args['email']->id || 'customer_new_account_activation' === $args['email']->id || 'customer_reset_password' === $args['email']->id ) ) {
		...
		$custom_shortcode->setOrderId( $args['order']->get_id(), $args['sent_to_admin'], $args );
		$custom_shortcode->shortCodesOrderDefined( $args['sent_to_admin'], $args );
	} elseif ( class_exists( 'WOOCOS_Email_Manager' ) ) {
		$custom_shortcode->setOrderId( $args['order']->get_id(), $args['sent_to_admin'], $args );
		$custom_shortcode->shortCodesOrderDefined( $args['sent_to_admin'], $args );
	} else {
	...
	<!DOCTYPE html>
		<html <?php language_attributes(); ?>>
			...
			<body <?php echo is_rtl() ? 'rightmargin' : 'leftmargin'; ?>="0" marginwidth="0" topmargin="0" marginheight="0" offset="0">
				<table style="background: <?php echo esc_attr( $emailBackgroundColor ); ?>" border="0" cellpadding="0" cellspacing="0" height="100%" width="100%" class="<?php echo esc_attr( 'yaymail-template-' . $yaymail_template ); ?>">
				<?php
