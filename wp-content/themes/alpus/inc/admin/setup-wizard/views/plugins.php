...
<form method="post">
	...
	$require = array_map(
		function( $item ) {
			return ALPHA_ENVATO_CODE ? $item['required'] : ( in_array( $item['slug'], array( ALPHA_CORE_SLUG, 'kirki', 'meta-box', 'elementor',  ) ) );
		},
		$plugins['all']
	);
				...
				<li data-slug="<?php echo esc_attr( $slug ); ?>"<?php echo 6 < $idx ? ' class="hidden"' : ''; ?>>
					...
					<div class="plugin-action">
						<label class="checkbox checkbox-inline">
							<input type="checkbox" name="setup-plugin"<?php echo ! ( ALPHA_ENVATO_CODE ? $plugin['required'] : ( in_array( $plugin['slug'], array( ALPHA_CORE_SLUG, 'kirki', 'meta-box', 'elementor',  ) ) ) ) ? '' : ' checked="checked"'; ?>>
