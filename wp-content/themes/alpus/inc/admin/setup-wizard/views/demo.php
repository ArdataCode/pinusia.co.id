	...
	<div id="theme-install-demos">
		<?php foreach ( $demos as $demo => $demo_details ) : ?>
			...
			<div class="theme <?php echo esc_attr( $demo_details['filter'] ); ?>">
				<div class="theme-wrapper<?php echo ALPHA_ENVATO_CODE ? '' : ( $demo_details['free'] ? ' demo-free' : ' demo-pro' ); ?>">
					<img class="theme-screenshot" src="<?php echo esc_url( $demo_details['img'] ); ?>" width="560" height="465" />
					...
					<?php if ( ALPHA_ENVATO_CODE || $demo_details['free'] ) { ?>
						<a class="demo-button demo-import fas fa-download" href="#" title="<?php esc_attr_e( 'Import', 'alpha' ); ?>"><?php esc_html_e( 'Import', 'alpha' ); ?></a>
					<?php } else { ?>	
						<a class="demo-button fas fa-unlock" href="<?php echo esc_url( ALPHA_GET_PRO_URI ) ?>" target="_blank" title="<?php esc_attr_e( 'Unlock', 'alpha' ); ?>"><?php esc_html_e( 'Import', 'alpha' ); ?></a>
					<?php } ?>
					...
					<?php if ( ! empty( $uninstalled_demo_plugins ) ) : ?>
							<?php foreach ( $uninstalled_demo_plugins as $plugin ) : ?>
								...
								<?php
								if ( ! ALPHA_ENVATO_CODE && $demo_details['free'] ) {
									if ( 'alpus-pro' == $plugin['slug'] || 'alpus-elementor-addon' == $plugin['slug'] ) {
										continue;
									}
								}
								?>
