...
<table class="shop_table cart wishlist_table wishlist_view traditional responsive<?php echo ! $no_interactions ? '' : ' no-interactions'; ?><?php echo ! $enable_drag_n_drop ? '' : ' sortable'; ?>" data-pagination="<?php echo esc_attr( $pagination ); ?>" data-per-page="<?php echo esc_attr( $per_page ); ?>" data-page="<?php echo esc_attr( $current_page ); ?>" data-id="<?php echo esc_attr( $wishlist_id ); ?>" data-token="<?php echo esc_attr( $wishlist_token ); ?>">
	...
	<tr>
		...
		if ( $show_price || $show_price_variations ) :
			...
			<th class="product-price">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Price', 'alpha' ) ); ?>
				</span>
			</th>
		<?php endif; ?>
		...
		if ( $show_quantity ) :
			...
			<th class="product-quantity">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_quantity_heading', esc_html__( 'Quantity', 'alpha' ) ); ?>
				</span>
			</th>
		<?php endif; ?>
		...
		if ( $show_stock_status ) :
			...
			<th class="product-stock-status">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock status', 'alpha' ) ); ?>
				</span>
			</th>
		<?php endif; ?>
		...
		if ( $enable_drag_n_drop ) :
			...
			<th class="product-arrange">
				<span class="nobr">
					<?php echo apply_filters( 'yith_wcwl_wishlist_view_arrange_heading', esc_html__( 'Arrange', 'alpha' ) ); ?>
				</span>
			</th>
		<?php endif; ?>
	</tr>
	...
	<tbody class="wishlist-items-wrapper<?php echo false != $wishlist && $wishlist->has_items() ? '' : ' wishlist-empty store-empty'; ?>">
	<?php
	if ( $wishlist && $wishlist->has_items() ) :
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<td class="product-thumbnail">
						...
						<?php if ( $show_remove_product ) : ?>
							<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove remove_from_wishlist fas fa-times" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'alpha' ) ); ?>"></a>
						<?php endif; ?>
					</td>
					...
					<?php if ( $show_stock_status ) : ?>
						<td class="product-stock-status">
							<?php echo 'out-of-stock' == $stock_status ? '<span class="wishlist-out-of-stock">' . apply_filters( 'yith_wcwl_out_of_stock_label', esc_html__( 'Out of stock', 'alpha' ) ) . '</span>' : '<span class="wishlist-in-stock">' . apply_filters( 'yith_wcwl_in_stock_label', esc_html__( 'In Stock', 'alpha' ) ) . '</span>'; ?>
						</td>
					<?php endif ?>

					<?php if ( $show_last_column ) : ?>
						<td class="product-add-to-cart">
							<div class="yith-product-actions">
								<!-- Date added -->
								<?php
								if ( $show_dateadded && $item->get_date_added() ) :
									echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on: %s', 'alpha' ), $item->get_date_added_formatted() ) . '</span>';
								endif;
								?>

								<!-- Quickview Button -->
								<?php echo '<button class="btn btn-quickview btn-outline btn-default btn-dark btn-md" data-mfp-src="' . alpha_get_product_featured_image_src( $product ) . '" data-product="' . $product->get_id() . '" title="' . esc_html__( 'Quick View', 'alpha' ) . '">' . esc_html__( 'Quick View', 'alpha' ) . '</button>'; ?>
								...
								if ( $repeat_remove_button ) :
									 ...
									 */
									?>
									<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ); ?>" class="remove_from_wishlist button btn-rounded" title="<?php echo esc_attr( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'alpha' ) ) ); ?>"><?php esc_html_e( 'Remove', 'alpha' ); ?></a>
								<?php endif; ?>
								...
								<?php if ( $move_to_another_wishlist && $available_multi_wishlist && count( $users_wishlists ) > 1 ) : ?>
									<?php if ( 'select' == $move_to_another_wishlist_type ) : ?>
										<select class="change-wishlist selectBox">
											<option value=""><?php esc_html_e( 'Move', 'alpha' ); ?></option>
											<?php
											foreach ( $users_wishlists as $wl ) :
												...
												<option value="<?php echo esc_attr( $wl->get_token() ); ?>">
													<?php printf( '%s - %s', $wl->get_formatted_name(), $wl->get_formatted_privacy() ); ?>
												</option>
												<?php
											endforeach;
											?>
									...
									<?php else : ?>
										<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
											<?php echo apply_filters( 'yith_wcwl_move_to_another_list_label', esc_html__( 'Move to another list &rsaquo;', 'alpha' ) ); ?>
										</a>
									<?php endif; ?>
								<?php endif; ?>
		...
		?>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>"><i class="a-icon-heart empty-icon"></i></td>
		</tr>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>" class="woocommerce-info"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products added to the wishlist.', 'alpha' ) ); ?></td>
		</tr>
		<tr>
			<td colspan="<?php echo esc_attr( $column_count ); ?>">
				<a class="button btn btn-dark" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
