...
<ul class="shop_table cart wishlist_table wishlist_view responsive mobile <?php echo ! $show_cb ? '' : 'with-checkbox'; ?>" data-pagination="<?php echo esc_attr( $pagination ); ?>" data-per-page="<?php echo esc_attr( $per_page ); ?>" data-page="<?php echo esc_attr( $current_page ); ?>" data-id="<?php echo esc_attr( $wishlist_id ); ?>" data-token="<?php echo esc_attr( $wishlist_token ); ?>">
	...
	if ( $wishlist && $wishlist->has_items() ) :
			...
			if ( $product && $product->exists() ) :
				...
				<li id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<div class="additional-info-wrapper">
						<?php if ( $show_quantity || $show_stock_status ) : ?>
							<table class="additional-info">
								...
								<?php if ( $show_stock_status ) : ?>
									<tr>
										<td class="label">
											<?php esc_html_e( 'Stock:', 'alpha' ); ?>
										</td>
										<td class="value">
											<?php echo 'out-of-stock' == $stock_status ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of stock', 'alpha' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'alpha' ) . '</span>'; ?>
										</td>
									</tr>
								<?php endif ?>
							</table>
						...
						<!-- Quickview Button -->
						<?php echo '<button class="btn btn-quickview btn-outline btn-dark btn-md" data-mfp-src="' . alpha_get_product_featured_image_src( $product ) . '" data-product="' . $product->get_id() . '" title="' . esc_html__( 'Quick View', 'alpha' ) . '">' . esc_html__( 'Quick View', 'alpha' ) . '</button>'; ?>
		...
		<p class="wishlist-empty">
			<?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products added to the wishlist', 'alpha' ) ); ?>
		</p>
		<?php
	endif;
