...
<?php
	...
	echo '<div class="cart-header">';
		...
		echo '<a class="btn btn-dark btn-link btn-icon-right btn-close" href="#"><i class="' . ALPHA_ICON_PREFIX . '-icon-long-arrow-right"></i><span class="sr-only">' . esc_html__( 'Cart', 'alpha' ) . '</span></a>';
	echo '</div>';
?>
...
<?php if ( ! WC()->cart->is_empty() ) : ?>
			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
				...
				<li class="woocommerce-mini-cart-item mini-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					<div class="cart-item-meta mini-item-meta">
						...
						if ( ! defined( 'ALPHA_PRO_VERSION' ) || ! alpha_get_option( 'cart_show_qty' ) ) {
							$product_quantity = $cart_item['quantity'];
						} else {
							$product_quantity = sprintf(
								'<input type="number" id="quantity_%1$s" name="%1$s" class="qty" value="%2$d" min="%3$d" max="%4$d" /><button class="quantity-minus %5$s-icon-minus" title="Minus"></button><button class="quantity-plus %5$s-icon-plus" title="Plus"></button>',
								$cart_item_key,
								$cart_item['quantity'],
								1,
					...
					echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						...
						sprintf(
							'<a href="%s" class="remove remove_from_cart_button" aria-label="%s" data-product_id="%s" data-cart_item_key="%s" data-product_sku="%s"><i class="' . ALPHA_ICON_PREFIX . '-icon-times-solid"></i></a>',
	...
	<div class="woocommerce-mini-cart_empty-content mini-basket-empty">
		<i class="<?php echo ALPHA_ICON_PREFIX; ?>-icon-cart-empty"></i>
		<div class="mini-basket-empty-content">
			...
			<a href="<?php echo wc_get_page_permalink( 'shop' ); ?>"><?php esc_html_e( 'Continue Shopping', 'alpha' ); ?><i class="<?php echo ALPHA_ICON_PREFIX; ?>-icon-angle-<?php echo is_rtl() ? 'left' : 'right'; ?>"></i></a>
		</div>
	</div>
